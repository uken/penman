...
class RecordTag < ActiveRecord::Base
  ...
  class << self
    ...
    def generate_update_seed(model, timestamp)
      ...
      base_file_path = File.join(Rails.root, 'db', 'migrate') #TODO throw this into a config
      ...
      File.open(seed_path, 'w') do |f|
        f.puts "class Seed#{model.name.pluralize}Updates"
        f.puts '  def up'
        ...
        touched_tags.each do |tag|
          ...
          f.print print_candidate_key(tag.record)
          f.puts  ')'

          f.print  "    record = #{model.name}.find_or_initialize_by("
          f.print attribute_string_from_hash(model, tag.candidate_key)
          ...
          f.print attribute_string_from_hash(model, column_hash)
          f.puts ')'
        end
    ...
    def generate_destroy_seed(model, timestamp)
      ...
      File.open(seed_path, 'w') do |f|
        f.puts "class Seed#{model.name.pluralize}Destroys"
        f.puts '  def up'
        f.puts '    # Generated by RecordTag.generate_seed'
        ...
        destroyed_tags.map(&:candidate_key).each do |record_candidate_key|
          f.puts "\n"
          f.print "    record = #{model.name}.find_by("
          f.puts "#{attribute_string_from_hash(model, record_candidate_key)})"
